...
`anchorage-windows-x64.exe` (+ `SHA256SUMS`, verified by the installers). Override the source with
...
Cut a release by pushing a tag: `git tag cli-v0.1.1 && git push origin cli-v0.1.1`.

```
anchorage --version
anchorage auth login                  # store default server (+ secret from env/stdin)
...
Use `--server http://localhost:3001` for local development.
