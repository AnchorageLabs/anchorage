...
async function main(): Promise<number> {
  ...
  if (task.value.task.type !== "bitbucket.pull_request.open") {
    emit(task.value, "agent.failed", "error", "Unsupported task type", {
      error: {
        code: "unsupported_task_type",
        message: `bitbucket-pr-opener only supports bitbucket.pull_request.open, got ${task.value.task.type}`,
      },
    });
    return ExitCode.UnsupportedTaskType;
  }

  emit(task.value, "agent.started", "info", "bitbucket-pr-opener started", { agentVersion });
  ...
  if (!token) {
    const msg = "Set BITBUCKET_TOKEN to push and open a Bitbucket pull request.";
    emit(task.value, "agent.failed", "error", msg, {
      error: { code: "missing_bitbucket_token", message: msg },
    });
    return ExitCode.MissingCapability;
  }

  const { workspacePath, repo, baseBranch, change } = input.value;

  const pushed = await ensureBranchPushed(task.value, workspacePath, change, token, "x-token-auth");
  if (!pushed.ok) return fail(task.value, pushed.failure);

  const title = buildTitle(change);
  const description = buildDescription(change);

  emit(task.value, "tool.requested", "info", "Creating Bitbucket pull request", {
    tool: "bitbucket.pullrequests.create",
    input: { repo, source: change.branchName, destination: baseBranch, title },
  });

  // Look for an existing PR BEFORE creating, so a retried/resumed/salvaged run
  // reuses its previous attempt's PR instead of opening a second one. The
  // pre-check fails open and a lookup failure after a failed create is reported
  // rather than read as "no PR exists" — see resolveOrCreatePr in @anchorage/sdk.
  const outcome = await resolveOrCreatePr<{ id: number; url: string; title: string }>({
    findOpen: () => findOpenPr(token, repo, change.branchName),
    create: () =>
      createPullRequest(token, repo, {
        title,
        source: { branch: { name: change.branchName } },
        destination: { branch: { name: baseBranch } },
        description,
      }),
    onReuse: (existing) =>
      emit(task.value, "tool.result", "info", `Reusing existing pull request #${existing.id}`, {
        tool: "bitbucket.pullrequests.list",
        success: true,
        output: { prId: existing.id, url: existing.url },
      }),
    onPreCheckFailed: (message) =>
      emit(task.value, "tool.result", "warn", "Could not check for an existing pull request", {
        tool: "bitbucket.pullrequests.list",
        success: false,
        output: { message },
      }),
  ...
  if ("failure" in outcome) {
    emit(task.value, "tool.result", "error", "Bitbucket PR creation failed", {
      tool: "bitbucket.pullrequests.create",
      success: false,
      output: { error: { code: "bitbucket_pr_create_failed", message: outcome.failure } },
    });
    emit(task.value, "agent.failed", "error", "Failed to create pull request", {
      error: { code: "bitbucket_pr_create_failed", message: outcome.failure },
    });
    return ExitCode.ExternalDependencyFailure;
  }
  const pr = outcome.pr;

  emit(task.value, "tool.result", "info", `Pull request #${pr.id} ready`, {
    tool: "bitbucket.pullrequests.create",
    success: true,
    output: { prNumber: pr.id, prUrl: pr.url, branchName: change.branchName },
  });
  ...
  emit(task.value, "agent.output", "info", "Pull request opened", output);
  ...
  emit(task.value, "agent.completed", "info", "bitbucket-pr-opener completed successfully", {
    prNumber: pr.id,
    prUrl: pr.url,
    branchName: change.branchName,
  });
...
async function createPullRequest(
  ...
  const response = await fetch(`${bitbucketApi}/2.0/repositories/${repo}/pullrequests`, {
    method: "POST",
    headers: {
  ...
  if (!response.ok || !isObject(data)) {
    const message =
      isObject(data) && isObject(data.error)
        ? (readString(data.error.message) ?? JSON.stringify(data))
        : `status ${response.status}`;
    throw new Error(`Bitbucket PR create failed: ${message}`);
  }
  return prFromJson(data);
}
...
async function findOpenPr(
  ...
  const response = await fetch(`${bitbucketApi}/2.0/repositories/${repo}/pullrequests?q=${q}`, {
    headers: { Authorization: `Bearer ${token}`, Accept: "application/json" },
  });
  const data = (await response.json().catch(() => null)) as JsonValue | null;
  if (!response.ok || !isObject(data) || !Array.isArray(data.values) || data.values.length === 0) {
    return null;
  }
  const first = data.values[0];
  return isObject(first) ? prFromJson(first) : null;
}

function prFromJson(data: JsonObject): { id: number; url: string; title: string } {
  const url =
    isObject(data.links) && isObject(data.links.html) ? readString(data.links.html.href) : null;
  return {
...
function resolveInput(task: TaskEnvelope): { ok: true; value: OpenerInput } | OpenerFailure {
  ...
  if (!workspacePath) {
    return failure(
      ...
      "bitbucket-pr-opener requires input.workspacePath pointing at the repository worktree.",
  ...
  if (!task.repository) {
    return failure(
      "missing_repository",
      "bitbucket-pr-opener requires repository.owner and repository.name in the task envelope.",
      ExitCode.InvalidInput,
    );
  }
  ...
  return {
    ...
    value: {
      workspacePath,
      repo: `${task.repository.owner}/${task.repository.name}`,
...
function resolveCodeChange(task: TaskEnvelope): { ok: true; value: CodeChange } | OpenerFailure {
  ...
  if (!artifact?.uri.startsWith("file://")) {
    return failure(
      ...
      "bitbucket-pr-opener requires input.codeChangeResult or a prior code.change.result artifact.",
      ExitCode.InvalidInput,
    );
  }
  try {
