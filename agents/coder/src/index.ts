...
3. Understand the code BEFORE editing. For a named symbol prefer the index tools over grep: locate_change tells you which files to open to change it; impact shows its full blast radius (callers + transitive dependents) before you touch a signature; find_references gives exact reference sites; repo_map orients you on an unfamiliar repo in one call. Use list_dir / read_file / grep / git_log for everything else. Do not edit a file you have not read. Before writing any call to an imported module, read that module's source first to verify its exact export names, parameter types, and return shape — never infer signatures from filenames or issue context.
