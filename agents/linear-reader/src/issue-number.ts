/**
 * Deriving a GitHub-style issue number from a Linear key.
 *
 ...
 *
 * This used to return **1** when the number could not be derived. A Linear
 * reference with no numeric tail (a UUID, which this agent accepts on purpose,
 * or `ENG-0`) therefore became issue 1, and the PR said `Closes #1`.
 *
...
export function numberFromIdentifier(identifier: string): number {
